...
;;
;; ---------------------------------------------------------------------------
;; NOTE: LOCALLY VENDORED + PATCHED COPY.  Do not replace with the ELPA version.
;;
;; This file lives in ~/.emacs.d/lisp/ (not elpa/) so package updates cannot
;; clobber the patches below.  It is self-contained: its only dependency is
;; the built-in `semantic/bovine/el'; it does NOT require srefactor.el.
;;
;; NB: keep this file WITHOUT a `lexical-binding: t' cookie -- the formatter
;; relies on dynamic binding for its parser-state variables.
;;
;; Local changes vs. upstream srefactor-20230504.617:
;;
;;   1. `srefactor--lisp-comment-formatter': extended the trailing newline
;;      guard so a standalone comment immediately after a form's head symbol
;;      forces a newline.  Without this, the formatter's "keep first arg on the
;;      head's line" space pulled the next token onto the comment line, silently
;;      commenting out real code.
;;
;;   2. `srefactor-lisp-format-region' (new command): formats every top-level
;;      sexp in the active region.  Upstream only offers point-based sexp/defun
;;      formatting, which errors ("Unbalanced parentheses") when point sits at
;;      the top level after a selection.
;; ---------------------------------------------------------------------------
;;
...
(defun srefactor-lisp-format-region (beg end)
  "Format each top-level sexp between BEG and END.
Unlike `srefactor-lisp-format-sexp', this honors the active region:
it walks every complete top-level form in BEG..END and formats each
one.  Forms are processed last-to-first so that reformatting an
earlier form does not invalidate the positions of later ones."
  (interactive "r")
  (save-excursion
    (let (starts)
      (goto-char beg)
    ...
    (when (and tok-end next-token-start)
      ...
      (when comment-token
        ...
        (when (or (srefactor--lisp-token-in-punctuation-p token)
                  ...
                  (string-match "[]}]" token-str)
                  ;; A comment right after the head symbol: the formatter will
                  ;; insert " " to keep the first arg on the head's line, which
                  ;; would pull that arg into the comment.  Force a newline.
                  (and (eq format-type 'multi-line)
